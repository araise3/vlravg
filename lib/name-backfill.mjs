...
const RAPID_NAME_BOUNCE_MS = 7 * 24 * 60 * 60 * 1000;
const SHORT_NAME_BLIP_MS = 24 * 60 * 60 * 1000;
const ESTABLISHED_NAME_MS = 7 * 24 * 60 * 60 * 1000;
...
export function compactMatchEvidence(rows) {
  ...
  // Match rosters can briefly disagree about an ID. A short B run between
  // established A evidence and a return to A is a stale-name blip even when
  // the returning A has only one match so far (including saved old scans).
  // Require an established A on at least one side and keep the window well
  // below Riot's usual 90-day rename cooldown; leave less certain reuse intact.
  for(let i=1;i<periods.length-1;){
    ...
    const blipMs=Date.parse(blip.last_seen)-Date.parse(blip.first_seen);
    const established=Date.parse(before.last_seen)-Date.parse(before.first_seen)>=ESTABLISHED_NAME_MS||
      Date.parse(after.last_seen)-Date.parse(after.first_seen)>=ESTABLISHED_NAME_MS;
    const singleRapid=blipMs===0&&bounceMs<48*60*60*1000;
    const shortAnchored=established&&blipMs<=SHORT_NAME_BLIP_MS&&bounceMs<RAPID_NAME_BOUNCE_MS;
    if(sameRiotId(before,after)&&Number.isFinite(blipMs)&&blipMs>=0&&
        Number.isFinite(bounceMs)&&bounceMs>=0&&(singleRapid||shortAnchored)){
