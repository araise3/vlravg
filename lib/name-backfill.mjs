...
const RAPID_NAME_BOUNCE_MS = 96 * 60 * 60 * 1000;
...
export function compactMatchEvidence(rows) {
  ...
  // Match rosters can briefly disagree about an ID. Remove a single-observation
  // bounce within 48 hours, or a short B run between two well-established A
  // runs. Keep less-certain short A -> B -> A sequences as possible name reuse.
  for(let i=1;i<periods.length-1;){
    ...
    const shortEstablished=established&&blipMs<=SHORT_NAME_BLIP_MS&&bounceMs<RAPID_NAME_BOUNCE_MS;
    if(sameRiotId(before,after)&&Number.isFinite(blipMs)&&blipMs>=0&&
        Number.isFinite(bounceMs)&&bounceMs>=0&&(singleRapid||shortEstablished)){
