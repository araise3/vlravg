import { compactMatchEvidence, mergeNameTimeline, parseCompactMatchEvidence } from './name-backfill.mjs';
...
export async function observeName(db, account, observedAt) {
  ...
  await db.batch([
    ...
    db.prepare(`INSERT INTO rr_players (puuid,region,platform,name,tag,updated_at)
      VALUES (?1,?2,?6,?3,?4,?5) ON CONFLICT(puuid) DO UPDATE SET
      region=COALESCE(excluded.region,rr_players.region),
      platform=COALESCE(rr_players.platform,excluded.platform),name=excluded.name,
      ...
      .bind(puuid, account.region || null, name, tag, stamp, account.platform || null),
  ]);
...
export async function readNameHistory(db, puuid) {
  ...
  const { results: matches } = await db.prepare(`SELECT name,tag,played_at FROM player_name_matches WHERE puuid=?1
    AND played_at<(SELECT MIN(first_seen) FROM player_name_history WHERE puuid=?1) ORDER BY played_at`).bind(puuid).all();
  const {results: compactRows}=await db.prepare(`SELECT evidence FROM player_name_backfill WHERE puuid=?1`).bind(puuid).all();
  const compact=(compactRows||[]).flatMap(row=>parseCompactMatchEvidence(row.evidence));
  return mergeNameTimeline(results || [],compactMatchEvidence([...(matches||[]),...compact]));
}
